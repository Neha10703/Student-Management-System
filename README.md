...
Controllers/StudentsController.cs — CRUD endpoints for students

Data/AppDbContext.cs — EF Core DbContext with Students and Users tables

DTOs/AuthDto.cs — RegisterDto, LoginDto, AuthResponseDto

DTOs/StudentDto.cs — StudentDto, StudentResponseDto
...
Repositories/IStudentRepository.cs — Repository interface

Repositories/StudentRepository.cs — EF Core repository implementation

Services/AuthService.cs — Registration and login business logic

Services/IStudentService.cs — Service interface

Services/StudentService.cs — Student business logic and DTO mapping

Services/TokenService.cs — JWT token generation

appsettings.json — Configuration for DB, JWT, Serilog
...
api/client.js — Axios instance with JWT interceptor and auto logout on 401

context/AuthContext.jsx — Global auth state (token, user, login, register, logout)

components/Layout.jsx — Protected route wrapper with sidebar and header
...
components/StudentForm.jsx — Shared Add/Edit student form with validation

pages/Login.jsx — Login and Register tabs

pages/Dashboard.jsx — Stats overview, recent students, course chart

pages/Students.jsx — Full CRUD table with search, filter, pagination
## Authentication Flow
